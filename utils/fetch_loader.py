...
def fetch_loader_book_wiki(model_names, args):
    ...
    if os.path.isfile('data/train_encoding_book_wiki.pickle'):
        ...
        print("=================== Data Loaded from Local Data Folder ===================")
    else:
        ...
        for model_name in model_names:
            tokenizer = AutoTokenizer.from_pretrained(model_name, add_prefix_space=True) ## changed here
            xlm_train_encoding = tokenizer(dataset_bookcorpus['train']["text"] + dataset_wiki['train']["text"], padding="longest",truncation=True)
            xlm_valid_encoding = tokenizer(dataset_bookcorpus['validation']["text"] + dataset_wiki['validation']["text"], padding="longest", truncation=True)
            xlm_test_encoding = tokenizer(dataset_bookcorpus['test']["text"] + dataset_wiki['validation']["text"], padding="longest", truncation=True)
