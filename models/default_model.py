    ...
    def train(self):
        ...
        for granularity in self.args.granularities:
            ...
            tokenizer = AutoTokenizer.from_pretrained(
                model_name, add_prefix_space=self.args.prefix_space
            )  ## changed here
            ...
            if granularity == "character":
                ...
                self.log.info(
                    "logits size: (%d, %d)", len(logits_tmp), len(logits_tmp[0])
                )
            elif granularity == "subword_30k" or granularity == "subword_50k":
                ## get subword logits
                pred_tmp = wnut_get_subword_logits(
                    model=model,
                    tokenized_wnut=tokenized_wnut,
                    prefix_space=self.args.prefix_space,
                    model_name=self.args.granularities_model[granularity],
            ...
            if count > 0:
                ...
                label_t = torch.stack(
                    (label_t, torch.argmax(torch.tensor(logits_tmp), dim=1)), 0
                )
            count += 1
