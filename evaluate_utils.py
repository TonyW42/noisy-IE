...
def wnut_get_predictions(model, tokenized_wnut, prefix_space, device):
    ...
    for samp in tokenized_wnut["test"]:
        ...
        encoded = model(input_ids=input_ids, attention_mask=attn_mask)
        ...
        assert (
            len(pred)
            == len(labels) & len(pred)
            == len(input_ids_squeezed) & len(labels)
            == len(input_ids_squeezed)
        )
    ...
    return {"pred": predicted, "label": label, "input_ids": input_ids_list}


def wnut_separate_char_prediction(pred_output, model_name):
    pred, label, input_ids = (
        pred_output["pred"],
        pred_output["label"],
        pred_output["input_ids"],
    )
    pred_word = []
    label_word = []
    ids_word = []
...
def wnut_char_to_word_first_letter(char_pred):
    pred_word, label_word, input_word = (
        char_pred["pred_word"],
        char_pred["label_word"],
        char_pred["input_word"],
    )
    ...
    return {"pred": pred, "word": inputs, "label": label}
...
def wnut_f1(pred, ref, average="macro"):
    ...
    return f1_metric.compute(predictions=pred, references=ref, average=average)


def wnut_evaluate_f1(
    model, tokenized_wnut, prefix_space, model_name, device, method="first letter"
):
...
def wnut_get_character_logits_raw(model, tokenized_wnut, prefix_space, device):
    ...
    for samp in tokenized_wnut["test"]:
        with torch.no_grad():
            ...
            if prefix_space:  ## added, remove trailing space
                pred = pred[1 : (len(pred) - 1)]
                input_ids_squeezed = input_ids_squeezed[
                    1 : (len(input_ids_squeezed) - 1)
                ]
                labels = labels[1 : (len(labels) - 1)]
...
def wnut_get_char_logits(
    model, tokenized_wnut, prefix_space, device, model_name, rule=3
):
    logits_raw = wnut_get_character_logits_raw(
        model, tokenized_wnut, prefix_space, device
    )
...
def wnut_get_subword_logits(
    ...
    predicted = [
        predicted_all[i] for i in range(0, len(label_all)) if i not in null_indices
    ]
    label = [label_all[i] for i in range(0, len(label_all)) if i not in null_indices]
    input_ids = [
        input_ids_all[i] for i in range(0, len(label_all)) if i not in null_indices
    ]
