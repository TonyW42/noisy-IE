...
def config_logging(log_dir):
    ...
    file_handler = logging.FileHandler(filename=os.path.join(log_dir, "experiment.log"))
        ...
        for data in tbar:
            ...
            if self.writer is not None:
                ...
                self.writer.add_scalar(
                    "train/micro/auc",
                    roc_auc_score(y, prob, average="micro"),
                    self.train_step,
                )
                if self.pred_thold is not None:
                    ...
                    self.writer.add_scalar(
                        "train/micro/precision", micros[0], self.train_step
                    )
                    self.writer.add_scalar(
                        "train/micro/recall", micros[1], self.train_step
                    )
    ...
    def train(self, cfg, trainloader, devloader=None, testloader=None):
        ...
        for i in range(cfg.n_epochs):
            ...
            if cfg.save.lower() == "true":
                if self.epoch % 20 == 0 or self.train_step % 2500 == 0:
        ...
        if self.mode == "dev":
            ...
            if self.writer is not None:
                ...
                if self.pred_thold is not None:
                    ...
                    self.writer.add_scalar(
                        "dev/macro/precision", macros[0], self.dev_step
                    )
                    ...
                    self.writer.add_scalar(
                        "dev/micro/precision", micros[0], self.dev_step
                    )
